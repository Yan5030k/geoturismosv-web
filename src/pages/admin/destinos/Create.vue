...
onMounted(async () => {
  const { data } = await supabase.from('categorias').select('*').eq('estado', true);
...
const guardar = async () => {
  ...
  try {
    ...
    const { error } = await supabase.from('destinos').insert(destinoPayload(form, imagen));
  ...
  <div class="min-h-screen bg-slate-50">
    ...
    <main class="mx-auto max-w-5xl px-6 py-10">
      ...
      <section class="mt-6 rounded-xl bg-white p-8 shadow">
        <h1 class="text-3xl font-bold">Nuevo destino turístico</h1>
        <p v-if="errorMessage" class="mt-4 text-red-600">{{ errorMessage }}</p>
        <form @submit.prevent="guardar" class="mt-8 grid gap-6 md:grid-cols-2">
          <div>
          ...
          <div class="flex gap-3 md:col-span-2">
            <button type="submit" :disabled="processing" class="rounded-full bg-[#168a1a] px-6 py-3 font-semibold text-white">Guardar destino</button>
