...
export async function render(root, route) {
  ...
  const favBlock = isLoggedIn()
    ? `<div class="mt-8 rounded-2xl bg-white/5 p-5 border border-white/10">
        <h2 class="text-xl font-bold text-white">${esc(t('detail.interested'))}</h2>
        <p class="mt-1 text-gray-400">${esc(t('detail.interested_desc'))}</p>
    ...
    : `<div class="mt-8 rounded-2xl border border-yellow-200/30 bg-yellow-50/10 p-5 text-yellow-100">
  ...
  const comercioHtml = comercios.length
    ? `<section class="mt-10">
        <h2 class="text-2xl font-black text-white">Comercio local</h2>
        <p class="mt-1 text-gray-400">Quien habita el destino: comida, artesanías y guías de San Miguel.</p>
        <div class="geo-carousel mt-5">
          ${comercios
            ...
            <article class="geo-comercio">
              ...
              <div class="p-4">
                <h3 class="font-black text-white">${esc(c.nombre)}</h3>
                <p class="mt-2 text-sm text-gray-300">${esc(c.descripcion)}</p>
                <p class="mt-3 text-sm font-bold text-[#f4a000]">${esc(formatearPrecio(c.costo_estimado, moneda, rates))}</p>
              </div>
            </article>`,
            )
            .join('')}
  ...
  const reviewForm = isLoggedIn()
    ? `<form id="review-form" class="geo-review-form mt-6 space-y-3">
        <label class="block text-sm text-gray-300">Estrellas
          <select name="estrellas" class="mt-1">
            ${[5, 4, 3, 2, 1].map((n) => `<option value="${n}">${n}</option>`).join('')}
          </select>
        </label>
        <label class="block text-sm text-gray-300">Tu visita
          <textarea name="comentario" required rows="3" placeholder="¿Qué viste, a quién conociste?"></textarea>
        </label>
        <p id="review-msg" class="text-sm text-[#7dffa0]"></p>
        <button class="rounded-full bg-[#168a1a] px-5 py-2 font-bold text-white">Publicar reseña</button>
      </form>`
    : `<p class="mt-4 text-gray-400">Inicia sesión para dejar tu reseña. <a href="#/entrar" data-link class="text-[#7dffa0] font-bold">Entrar</a></p>`;

  root.innerHTML = `
    <div class="min-h-screen geo-territory">
      ${publicNav()}
      <main class="mx-auto max-w-5xl px-6 py-10">
        <a href="#/destinos" data-link class="font-semibold text-[#7dffa0]">${esc(t('detail.back'))}</a>
        <article class="mt-6 overflow-hidden rounded-[28px] bg-[#132228] border border-white/10">
          <div class="relative">
          ...
          <div class="p-8">
            <p class="font-semibold text-[#7dffa0]">${esc(tDb(data.categoria, 'nombre'))}</p>
            <h1 class="mt-2 text-4xl font-black text-white">${esc(tDb(data, 'nombre'))}</h1>
            <p class="mt-2 text-gray-400">${esc(tDb(data, 'ubicacion'))}</p>
            <div class="mt-4">${(data.rango_edad || []).map((r) => `<span class="geo-chip">${esc(r)}</span>`).join('')}</div>
            <div class="mt-6 grid gap-4 md:grid-cols-3">
              <div class="rounded-xl bg-white/5 p-4"><strong class="text-[#7dffa0]">${esc(t('detail.cost'))}</strong><p class="mt-1 text-white">${esc(formatearPrecio(data.costo_estimado, moneda, rates))}</p></div>
              <div class="rounded-xl bg-white/5 p-4"><strong class="text-[#7dffa0]">${esc(t('detail.hours'))}</strong><p class="mt-1 text-white">${esc(tDb(data, 'dias_atencion') || t('detail.hours_unspecified'))}</p><p class="text-gray-300">${data.hora_apertura && data.hora_cierre ? `${esc(data.hora_apertura)} - ${esc(data.hora_cierre)}` : esc(t('detail.time_unspecified'))}</p></div>
              <div class="rounded-xl bg-white/5 p-4"><strong class="text-[#f4a000]">${esc(t('detail.status'))}</strong><p class="mt-1 text-white">${esc(data.estado ? t('detail.available') : t('detail.unavailable'))}</p></div>
            </div>
            <section class="mt-8">
              <div class="flex items-center justify-between gap-4">
                <h2 class="text-2xl font-bold text-white">${esc(t('detail.description'))}</h2>
              ...
              <p class="mt-4 leading-relaxed text-gray-200 whitespace-pre-wrap">${esc(tDb(data, 'descripcion'))}</p>
            </section>
            <section class="mt-6">
              <h2 class="text-2xl font-bold text-white">${esc(t('detail.location'))}</h2>
              <div class="mt-3 grid gap-4 md:grid-cols-2">
                <div class="rounded-xl bg-white/5 p-4"><strong class="text-[#7dffa0]">${esc(t('detail.department'))}</strong><p class="text-white">${esc(data.departamento || t('detail.unspecified'))}</p></div>
                <div class="rounded-xl bg-white/5 p-4"><strong class="text-[#7dffa0]">${esc(t('detail.municipality'))}</strong><p class="text-white">${esc(data.municipio || t('detail.unspecified'))}</p></div>
              </div>
              <div class="mt-4 rounded-xl bg-white/5 p-4"><strong class="text-white">${esc(t('detail.address'))}</strong><p class="text-gray-300">${esc(tDb(data, 'direccion') || t('detail.address_unspecified'))}</p></div>
              <div class="mt-6">${maps}</div>
            </section>
            <section class="mt-6">
              <h2 class="text-2xl font-bold text-white">${esc(t('detail.contact_info'))}</h2>
              <div class="mt-3 grid gap-4 md:grid-cols-3">
                <div class="rounded-xl bg-white/5 p-4"><strong class="text-[#7dffa0]">${esc(t('detail.contact'))}</strong><p class="text-white">${esc(tDb(data, 'contacto') || t('detail.unspecified'))}</p></div>
                <div class="rounded-xl bg-white/5 p-4"><strong class="text-[#7dffa0]">${esc(t('detail.phone'))}</strong><p class="text-white">${esc(data.telefono || t('detail.unspecified'))}</p></div>
                <div class="rounded-xl bg-white/5 p-4"><strong class="text-[#f4a000]">${esc(t('detail.website'))}</strong>${data.sitio_web ? `<a href="${esc(data.sitio_web)}" class="text-[#7dffa0] underline" target="_blank" rel="noopener">${esc(t('detail.visit_website'))}</a>` : `<p class="text-white">${esc(t('detail.unspecified'))}</p>`}</div>
              </div>
            </section>
            <section class="mt-6">
              <h2 class="text-2xl font-bold text-white">${esc(t('detail.recommendations'))}</h2>
              <p class="mt-2 whitespace-pre-wrap text-gray-200">${esc(tDb(data, 'recomendaciones'))}</p>
            </section>
            ${comercioHtml}
            <section class="mt-10">
              <h2 class="text-2xl font-black text-white">Reseñas de la comunidad</h2>
              <div class="mt-4 space-y-4">
                ${
                  ...
                  <article class="rounded-2xl bg-white/5 p-4 border border-white/10">
                    <p class="text-sm font-bold text-white">${esc(r.perfil?.nombre || 'Viajero')}</p>
                    <div class="mt-1">${estrellasHtml(r.estrellas)}</div>
                    <p class="mt-2 text-gray-200">${esc(r.comentario)}</p>
                  </article>`,
                        )
                        .join('')
                    : `<p class="text-gray-400">Sé el primero en contar esta visita.</p>`
                }
              </div>
              ${reviewForm}
            </section>
